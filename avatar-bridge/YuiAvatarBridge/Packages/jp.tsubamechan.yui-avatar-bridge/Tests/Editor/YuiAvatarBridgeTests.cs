...
namespace Yui.AvatarBridge.Editor.Tests
{
    ...
    public sealed class YuiAvatarBridgeTests
    {
        ...
        [Test]
        public void BasicMaterialConversionPreservesSourceAndOnlyCarriesBasicAppearance()
        {
            var source = new Material(Shader.Find("Standard"));
            var texture = new Texture2D(2,2); source.mainTexture = texture; source.color = Color.red;
            source.SetFloat("_Glossiness", .8f);
            Material converted = null;
            try {
                converted = YuiAvatarBasicMaterials.Convert(source);
                Assert.AreNotSame(source, converted); Assert.AreSame(texture, converted.mainTexture);
                Assert.AreEqual(Color.red, converted.color); Assert.AreEqual(.8f, source.GetFloat("_Glossiness"));
                Assert.AreEqual(0, converted.GetFloat("_Glossiness"));
            } finally { Object.DestroyImmediate(source); Object.DestroyImmediate(texture); if(converted != null) Object.DestroyImmediate(converted); }
        }

        [TestCase("nadena.dev.modular_avatar.core.ModularAvatarMergeArmature", true)]
        [TestCase("nadena.dev.ndmf.runtime.VirtualLens", true)]
        [TestCase("VRC.SDK3.Dynamics.PhysBone.Components.VRCPhysBone", false)]
        [TestCase("UnityEngine.Animator", false)]
        public void SourceBakeDetectionDistinguishesBuildPlugins(string type, bool expected)
        { Assert.AreEqual(expected, YuiAvatarBridgeAnalyzer.IsBuildTimeComponent(type)); }
