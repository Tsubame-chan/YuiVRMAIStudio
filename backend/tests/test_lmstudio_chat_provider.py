...
def test_provider_router_returns_litert_lm_chat_provider_alias() -> None:
    provider = ProviderRouter(Settings(chat_provider="litert_lm")).chat()
...
@pytest.mark.anyio
async def test_litert_lm_chat_alias_uses_litert_server_settings() -> None:
    captured_request: httpx.Request | None = None
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"choices": [{"message": {"content": "うん、ローカルで話せてるよ。"}}]},
        )

    provider = LMStudioChatProvider(
        Settings(
            chat_provider="litert_lm",
            litert_lm_base_url="https://litert.example.test/v1",
            litert_lm_chat_model="gemma4-e4b,gpu",
        ),
        transport=httpx.MockTransport(handler),
    )
    ...
    assert response.text == "うん、ローカルで話せてるよ。"
    assert captured_request is not None
    assert captured_request.url == "https://litert.example.test/v1/chat/completions"
    payload = json.loads(captured_request.content)
    assert payload["model"] == "gemma4-e4b,gpu"
