...
Yui VRM AI Studio is a local AI avatar studio for Windows. It pairs a Unity
...
Voice output currently focuses on Japanese speech because it uses local
VOICEVOX Engine. The UI and text chat can still be used in English.

## Features

- UnityChan Default avatar and local custom `.vrm` import.
- VRM 1.0 and VRM 0.x support.
- Text chat through a local FastAPI backend.
...
- Experimental realtime modes in the Unity UI:
  - realtime conversation
  - realtime conversation with VOICEVOX playback
  - realtime translation
  - image/screen-context conversation experiments
...
are left off. If you only want to try the main features, 5 USD of API credit is
usually enough to play with chat, voice input, image input, and translation.
Longer realtime sessions, image-heavy use, or repeated audio tests can increase
cost, so keep an eye on your OpenAI usage page.
...
- Cannot stop services normally: press Enter in the launcher window first. Use
  `Stop_Yui_Backend_And_VOICEVOX.bat` only as a force-stop helper.
