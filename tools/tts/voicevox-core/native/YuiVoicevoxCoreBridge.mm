...
static BOOL YuiVoicevoxEnsureSynthesizer(NSString *dictPath)
{
    ...
    Dl_info location;
    dladdr((const void *)&YuiVoicevoxEnsureSynthesizer, &location);
    NSString *directory = [[NSString stringWithUTF8String:location.dli_fname] stringByDeletingLastPathComponent];
    NSString *runtimePath = [directory stringByAppendingPathComponent:@"libvoicevox_onnxruntime.1.17.3.dylib"];
    VoicevoxLoadOnnxruntimeOptions runtimeOptions = voicevox_make_default_load_onnxruntime_options();
    runtimeOptions.filename = [runtimePath UTF8String];
    VoicevoxResultCode result = voicevox_onnxruntime_load_once(runtimeOptions, &yuiVoicevoxOrt);
