...
MODEL_REPO="${YUI_DESKTOP_LITERT_LM_REPO:-litert-community/gemma-4-E4B-it-litert-lm}"
MODEL_FILE="${YUI_DESKTOP_LITERT_LM_FILE:-gemma-4-E4B-it.litertlm}"
...
"$VENV_DIR/bin/litert-lm" run \
  ...
  --backend=cpu \
  --cache=no \
  --prompt="ok" >/tmp/yui-litert-desktop-asset-warmup.log 2>&1 || true
...
echo "Prepared desktop local AI model asset: $OUTPUT_DIR/$MODEL_FILE"
